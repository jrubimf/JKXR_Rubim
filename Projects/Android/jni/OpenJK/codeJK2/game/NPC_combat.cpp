...
void G_AngerAlert(gentity_t *self) {
    if (self && self->NPC && (self->NPC->scriptFlags & SCF_NO_GROUPS)) {//I'm not a team playa...
        return;
    }
    if (!TIMER_Done(self,
                    "interrogating")) {//I'm interrogating, don't wake everyone else up yet... FIXME: this may never wake everyone else up, though!
        return;
    }
...
qboolean G_TeamEnemy(
        gentity_t *self) {//FIXME: Probably a better way to do this, is a linked list of your teammates already available?
    int i;
    gentity_t *ent;
    ...
    for (i = 1; i < MAX_GENTITIES; i++) {
        ...
        if (ent->client->playerTeam != self->client->playerTeam) {//ent is not on my team
            continue;
        }

        if (ent->enemy) {//they have an enemy
            if (!ent->enemy->client || ent->enemy->client->playerTeam !=
                                       self->client->playerTeam) {//the ent's enemy is either a normal ent or is a player/NPC that is not on my team
                return qtrue;
            }
        }
    }
...
void G_AttackDelay(gentity_t *self, gentity_t *enemy) {
    if (enemy && self->client &&
        self->NPC) {//delay their attack based on how far away they're facing from enemy
        vec3_t fwd, dir;
        int attDelay;

        VectorSubtract(self->client->renderInfo.eyePoint, enemy->currentOrigin,
                       dir);//purposely backwards
        VectorNormalize(dir);
        AngleVectors(self->client->renderInfo.eyeAngles, fwd, NULL, NULL);
        //dir[2] = fwd[2] = 0;//ignore z diff?

        attDelay = (4 - g_spskill->integer) *
                   500;//initial: from 1000ms delay on hard to 2000ms delay on easy
        if (self->client->playerTeam == TEAM_PLAYER) {//invert
            attDelay = 2000 - attDelay;
        }
        attDelay += floor((DotProduct(fwd, dir) + 1.0f) *
                          2000.0f);//add up to 4000ms delay if they're facing away
        ...
        switch (self->client->NPC_class) {
            ...
            case CLASS_STORMTROOPER://stormtroopers shoot sooner
                if (self->NPC->rank >= RANK_LT) {//officers shoot even sooner
                    attDelay -= Q_irand(500, 1500);
                } else {//normal stormtroopers don't have as fast reflexes as officers
		...
		*/
        ...
        switch (self->s.weapon) {
            ...
            case WP_BRYAR_PISTOL:
                break;
            case WP_BLASTER:
                if (self->NPC->scriptFlags & SCF_ALT_FIRE) {//rapid-fire blasters
                    attDelay += Q_irand(0, 500);
                } else {//regular blaster
                    attDelay -= Q_irand(0, 500);
                }
                break;
            case WP_BOWCASTER:
		...
		*/
        ...
        if (self->client->playerTeam == TEAM_PLAYER) {//clamp it
            if (attDelay > 2000) {
                attDelay = 2000;
            }
        }
...
void G_SetEnemy(gentity_t *self, gentity_t *enemy) {
    ...
    if (self->NPC->confusionTime > level.time) {//can't pick up enemies if confused
        return;
    }
...
//	}
    ...
    if (self->client && self->NPC && enemy->client &&
        enemy->client->playerTeam == self->client->playerTeam) {//Probably a damn script!
        if (self->NPC->charmedTime > level.time) {//Probably a damn script!
            return;
        }
    }
    ...
    if (self->enemy == NULL) {
        ...
        if (G_ActivateBehavior(self, BSET_ANGER)) {
        } else if (self->client && enemy->client &&
            ...
            {
                if (!G_TeamEnemy(self)) {//team did not have an enemy previously
        ...
        if (self->s.weapon == WP_BLASTER || self->s.weapon == WP_REPEATER ||
            ...
            || self->s.weapon == WP_BOWCASTER) {//Hmm, how about sniper and bowcaster?
            ...
            if (self->client->playerTeam == TEAM_PLAYER) {
                ...
                if (self->client->NPC_class == CLASS_IMPWORKER) {
                    minErr = 15;
                    maxErr = 30;
                } else if (self->client->NPC_class == CLASS_STORMTROOPER && self->NPC &&
        ...
        if (Q_stricmp("desperado", self->NPC_type) != 0 &&
            Q_stricmp("paladin", self->NPC_type) != 0) {//special holodeck enemies exception
            if (!(self->client->ps.eFlags & EF_FORCE_GRIPPED)) {//gripped people can't call for help
                G_AngerAlert(self);
            }
        }
        ...
        if (self->client->ps.weapon == WP_NONE && !Q_strncmp(self->NPC_type, "imp", 3) &&
...
*/
void NPC_ApplyWeaponFireDelay(void) {
    if (NPC->attackDebounceTime >
        level.time) {//Just fired, if attacking again, must be a burst fire, so don't add delay
    ...
    switch (client->ps.weapon) {
        ...
        case WP_THERMAL:
            if (client->ps.clientNum) {//NPCs delay...
        ...
        case WP_MELEE:
            if (!PM_DroidMelee(client->NPC_class)) {//FIXME: should be unique per melee anim
...
qboolean ShotThroughGlass(trace_t *tr, gentity_t *target, vec3_t spot, int mask) {
    ...
    if (hit != target && EntIsGlass(hit)) {//ok to shoot through breakable glass
...
qboolean CanShoot(gentity_t *ent, gentity_t *shooter) {
    ...
    CalcEntitySpot(ent, SPOT_ORIGIN,
                   spot);        //FIXME preferred target locations for some weapons (feet for R/L)
...
//MCG - Begin
    else {//ok, can't hit them in center, try their head
...
void NPC_CheckPossibleEnemy(gentity_t *other, visibility_t vis) {
    ...
    if (!NPC->enemy) {//only take an enemy if you don't have one yet
...
float NPC_MaxDistSquaredForWeapon(void) {
    if (NPCInfo->stats.shootDistance > 0) {//overrides default weapon dist
...
*/
        case WP_SABER:
            if (NPC->client &&
                NPC->client->ps.saberLength) {//FIXME: account for whether enemy and I are heading towards each other!
                return (NPC->client->ps.saberLength + NPC->maxs[0] * 1.5) *
                       (NPC->client->ps.saberLength + NPC->maxs[0] * 1.5);
            } else {
                return 48 * 48;
            }
            break;
...
qboolean NPC_EnemyTooFar(gentity_t *enemy, float dist, qboolean toShoot) {
    ...
    if (!toShoot) {//Not trying to actually press fire button with this check
        if (NPC->client->ps.weapon == WP_SABER) {//Just have to get to him
            return qfalse;
        }
    }
...
*/
gentity_t *
NPC_PickEnemy(gentity_t *closestTo, int enemyTeam, qboolean checkVis, qboolean findPlayersFirst,
    ...
    if (NPCInfo->behaviorState == BS_STAND_AND_SHOOT ||
        NPCInfo->behaviorState ==
        BS_HUNT_AND_KILL) {//Formations guys don't require inFov to pick up a target
        ...
        if (newenemy->client && !(newenemy->flags & FL_NOTARGET) &&
            ...
            if (newenemy->health > 0) {
                if (enemyTeam == TEAM_PLAYER || newenemy->client->playerTeam == enemyTeam ||
                    (enemyTeam == TEAM_PLAYER)) {//FIXME:  check for range and FOV or vis?
                    if (newenemy !=
                        NPC->lastEnemy) {//Make sure we're not just going back and forth here
                        if (gi.inPVS(newenemy->currentOrigin, NPC->currentOrigin)) {
                            if (NPCInfo->behaviorState == BS_INVESTIGATE ||
                                ...
                                if (!NPC->enemy) {
                                    if (!InVisrange(newenemy)) {
                                        failed = qtrue;
                                    } else if (NPC_CheckVisibility(newenemy, CHECK_360 | CHECK_FOV |
                                                                             CHECK_VISRANGE) !=
                                               VIS_FOV) {
                                        failed = qtrue;
                                    }
                            ...
                            if (!failed) {
                                VectorSubtract(closestTo->currentOrigin, newenemy->currentOrigin,
                                               diff);
                                relDist = VectorLengthSquared(diff);
                                if (newenemy->client->hiddenDist > 0) {
                                    if (relDist > newenemy->client->hiddenDist *
                                                  newenemy->client->hiddenDist) {
                                        //out of hidden range
                                        if (VectorLengthSquared(
                                                newenemy->client->hiddenDir)) {//They're only hidden from a certain direction, check
                                            float dot;
                                            VectorNormalize(diff);
                                            dot = DotProduct(newenemy->client->hiddenDir, diff);
                                            if (dot >
                                                0.5) {//I'm not looking in the right dir toward them to see them
                                                failed = qtrue;
                                            } else {
                                                Debug_Printf(debugNPCAI, DEBUG_LEVEL_INFO,
                                                             ...
                                                             vtos(newenemy->client->hiddenDir),
                                                             vtos(diff), dot);
                                            }
                                        } else {
                                            failed = qtrue;
	...
	*/
    ...
    for (entNum = 0; entNum < globals.num_entities; entNum++) {
        ...
        if (newenemy != NPC && (newenemy->client || newenemy->svFlags & SVF_NONNPC_ENEMY) &&
            !(newenemy->flags & FL_NOTARGET) && !(newenemy->s.eFlags & EF_NODRAW)) {
            if (newenemy->health > 0) {
                if ((newenemy->client && newenemy->client->playerTeam == enemyTeam) ||
                    (!newenemy->client && newenemy->noDamageTeam ==
                                          enemyTeam)) {//FIXME:  check for range and FOV or vis?
                    if (NPC->client->playerTeam == TEAM_PLAYER &&
                        enemyTeam == TEAM_PLAYER) {//player allies turning on ourselves?  How?
                        if (newenemy->s.number) {//only turn on the player, not other player allies
                            continue;
                        }
                    }
                    ...
                    if (newenemy !=
                        ...
                        if (newenemy->client && newenemy->client->hiddenDist > 0) {
                            if (relDist >
                                newenemy->client->hiddenDist * newenemy->client->hiddenDist) {
                                //out of hidden range
                                if (VectorLengthSquared(
                                        newenemy->client->hiddenDir)) {//They're only hidden from a certain direction, check
                                    float dot;
    ...
    if (findClosest) {//FIXME: you can pick up an enemy around a corner this way.
        return closestEnemy;
    }
...
gentity_t *
NPC_PickAlly(qboolean facingEachOther, float range, qboolean ignoreGroup, qboolean movingOnly) {
    gentity_t *ally = NULL;
        ...
        if (ally->client) {
            if (ally->health > 0) {
                if (ally->client && (ally->client->playerTeam == NPC->client->playerTeam ||
                                     ...
                                     TEAM_ENEMY))// && ally->client->playerTeam == TEAM_DISGUISE ) ) )
                {//if on same team or if player is disguised as your team
                    ...
                    if (movingOnly && ally->client &&
                        NPC->client) {//They have to be moving relative to each other
                    ...
                    if (relDist < bestDist) {
                        if (facingEachOther) {
                            ...
                            if (dot < 0.5) {//Not facing in dir to me
                                continue;
                            }
                            ...
                            if (dot > -0.5) {//I'm not facing opposite of dir to me
                                continue;
                            }
                            //I am facing him
                        }
...
gentity_t *NPC_CheckEnemy(qboolean findNew, qboolean tooFarOk, qboolean setEnemy) {
    ...
    if (NPC->svFlags & SVF_LOCKEDENEMY) {//keep this enemy until dead
        if (NPC->enemy) {
            if ((!NPC->NPC && !(NPC->svFlags & SVF_NONNPC_ENEMY)) || NPC->enemy->health >
                                                                     0) {//Enemy never had health (a train or info_not_null, etc) or did and is now dead (NPCs, turrets, etc)
                return NULL;
            }
        }
    ...
    if (NPC->enemy) {
        if (NPC_EnemyTooFar(NPC->enemy, 0, qfalse)) {
            if (findNew) {//See if there is a close one and take it if so, else keep this one
                forcefindNew = qtrue;
        ...
        } else if (!gi.inPVS(NPC->currentOrigin,
                             NPC->enemy->currentOrigin)) {//FIXME: should this be a line-of site check?
            ...
            if (NPC->enemy->client &&
                NPC->enemy->client->hiddenDist) {//He ducked into shadow while we weren't looking
                //Drop enemy and see if we should search for him
                NPC_LostEnemyDecideChase();
            } else {//If we're not chasing him, we need to lose him
    ...
    if (NPCInfo->defendEnt) {//Trying to protect someone
        if (NPCInfo->defendEnt->health >
            0) {//Still alive, We presume we're close to them, navigation should handle this?
            if (NPCInfo->defendEnt->enemy) {//They were shot or acquired an enemy
                if (NPC->enemy !=
                    NPCInfo->defendEnt->enemy) {//They have a different enemy, take it!
                    newEnemy = NPCInfo->defendEnt->enemy;
                    if (setEnemy) {
                        G_SetEnemy(NPC, NPCInfo->defendEnt->enemy);
                    }
                }
            } else if (NPC->enemy == NULL) {//We don't have an enemy, so find closest to defendEnt
                closestTo = NPCInfo->defendEnt;
            }
        }
    }

    if (!NPC->enemy || (NPC->enemy && NPC->enemy->health <= 0) ||
        forcefindNew) {//FIXME: NPCs that are moving after an enemy should ignore the can't hit enemy counter- that should only be for NPCs that are standing still
        ...
        if (NPC->client->enemyTeam) {
            ...
            newEnemy = NPC_PickEnemy(closestTo, NPC->client->enemyTeam, qtrue, qfalse,
                                     qtrue);//3rd parm was (NPC->enemyTeam == TEAM_STARFLEET)
            if (newEnemy) {
                foundenemy = qtrue;
                if (setEnemy) {
...
//	if ( NPC->client->playerTeam == TEAM_SCAVENGERS )
    if (NPC->s.weapon == WP_BLASTER ||
        NPC->s.weapon == WP_BLASTER_PISTOL) // any other guns to check for?
    {
        vec3_t mins = {-2, -2, -2};
        vec3_t maxs = {2, 2, 2};
...
int NPC_ShotEntity(gentity_t *ent, vec3_t impactPos) {
    ...
    if (NPC->s.weapon == WP_THERMAL) {//thermal aims from slightly above head
        ...
        gi.trace(&tr, muzzle, vec3_origin, vec3_origin, end, NPC->s.number, MASK_SHOT, G2_NOCOLLIDE,
                 0);
        VectorCopy(tr.endpos, muzzle);
    } else {
        CalcEntitySpot(NPC, SPOT_WEAPON, muzzle);
...
//	if ( NPC->client->playerTeam == TEAM_SCAVENGERS )
    ...
    if (impactPos) {//they want to know *where* the hit would be, too
...
qboolean NPC_EvaluateShot(int hit, qboolean glassOK) {
    ...
    if (hit == NPC->enemy->s.number || (&g_entities[hit] != NULL && (g_entities[hit].svFlags &
...
//	qboolean	duck_ok = qfalse;
    ...
    if (NPC_EnemyTooFar(NPC->enemy, distanceToEnemy * distanceToEnemy,
                        qtrue)) {//Too far away?  Do not attack
        return qfalse;
    }

    if (client->fireDelay > 0) {//already waiting for a shot to fire
    ...
    if (enemyVisibility >= VIS_FOV) {//He's in our FOV
        ...
        if (NPC->enemy->client) {
            if (NPC->enemy->enemy == NPC) {
                if (NPC->enemy->client->buttons &
                    BUTTON_ATTACK) {//FIXME: determine if enemy fire angles would hit me or get close
        ...
        if (attack_ok) {
			...
			*/
            ...
            if (traceEnt == NPC->enemy || (traceEnt->client && NPC->client->enemyTeam &&
                ...
                if (NPC->client->playerTeam) {
                    if (traceEnt && traceEnt->client && traceEnt->client->playerTeam) {
                        if (NPC->client->playerTeam ==
                            traceEnt->client->playerTeam) {//Don't shoot our own team
                            attack_ok = qfalse;
                        }
                    }
                }
        ...
        if (attack_ok) {
            ...
            if (!dead_on) {//We're not going to hit him directly, try a suppressing fire
                ...
                if (traceEnt &&
                    (traceEnt->health <= 30 || EntIsGlass(traceEnt))) {//easy to kill - go for it
                    if (traceEnt->e_DieFunc == dieF_ExplodeDeath_Wait &&
                        traceEnt->splashDamage) {//going to explode, don't shoot if close to self
                        VectorSubtract(NPC->currentOrigin, traceEnt->currentOrigin, diff);
                        if (VectorLengthSquared(diff) <
                            traceEnt->splashRadius * traceEnt->splashRadius) {//Too close to shoot!
                            attack_ok = qfalse;
                        } else {//Hey, it might kill him, do it!
                            attack_scale *= 2;//
                        }
                    }
                } else {
                    ...
                    if (aim_off >
                        Q_flrand(0.0f, 1.0f) * max_aim_off)//FIXME: use aim value to allow poor aim?
                    {
                        attack_scale *= 0.75;
...
void CP_FindCombatPointWaypoints(void) {
    for (int i = 0; i < level.numCombatPoints; i++) {
        level.combatPoints[i].waypoint = NAV_FindClosestWaypointForPoint(
                level.combatPoints[i].origin);
#ifndef FINAL_BUILD
...
static int NPC_CollectCombatPoints(const vec3_t origin, const float radius, combatPoint_m &points,
                                   const int flags) {
    ...
    for (int i = 0; i < level.numCombatPoints; i++) {
        ...
        if (flags & CP_NO_PVS) {//must not be within PVS of mu current origin
...
int NPC_FindCombatPoint(const vec3_t position, const vec3_t avoidPosition, vec3_t enemyPosition,
                        const int flags, float avoidDist, const int ignorePoint) {
    ...
    if ((flags & CP_HAS_ROUTE) || (flags & CP_NEAREST)) {//going to be doing macro nav tests
    ...
    if (flags & CP_NO_PVS) {//much larger radius since most will be dropped?
    ...
    STL_ITERATE(cpi, points) {
        ...
        if ((flags & CP_COVER) && (NPC_ClearLOS(level.combatPoints[i].origin, enemyPosition) ==
                                   qtrue))//Used to use NPC->enemy
            continue;
        ...
        if (flags & CP_CLEAR) {
            ...
            if (NPC->s.weapon == WP_THERMAL) {//horizontal
        ...
        if ((flags & CP_AVOID) && (DistanceSquared(level.combatPoints[i].origin, position) <
                                   avoidDist))//was using MIN_AVOID_DISTANCE_SQUARED, not passed in avoidDist
            continue;
        ...
        if (flags & CP_RETREAT) {
            if (flags & CP_HORZ_DIST_COLL) {
                if ((*cpi).first <
                    DistanceHorizontalSquared(position, enemyPosition)) {//it's closer, don't use it
                    continue;
                }
            } else {
                if ((*cpi).first <
                    DistanceSquared(position, enemyPosition)) {//it's closer, don't use it
                    continue;
                }
            }
        }
        ...
        gi.trace(&tr, level.combatPoints[i].origin, NPC->mins, NPC->maxs,
                 level.combatPoints[i].origin, NPC->s.number, NPC->clipmask, G2_NOCOLLIDE, 0);
        ...
        if (flags & CP_HAS_ROUTE) {
			...
			*/
            ...
            if (waypoint == WAYPOINT_NONE || level.combatPoints[i].waypoint == WAYPOINT_NONE ||
                navigator.GetBestNodeAltRoute(waypoint, level.combatPoints[i].waypoint) ==
                WAYPOINT_NONE) {//can't possibly have a route to any OR can't possibly have a route to this one OR don't have a route to this one
                if (!NAV_ClearPathToPoint(NPC, NPC->mins, NPC->maxs, level.combatPoints[i].origin,
                                          NPC->clipmask,
                                          ENTITYNUM_NONE)) {//don't even have a clear straight path to this one
                    continue;
                }
            }
        }
        ...
        if (flags & CP_NEAREST && waypoint != WAYPOINT_NONE &&
            level.combatPoints[i].waypoint != WAYPOINT_NONE) {
            ...
            if (gi.inPVS(found->currentOrigin, NPC->currentOrigin)) {
                ...
                if (dist < bestDist) {
                    if (!navigator.GetBestPathBetweenEnts(NPC, found, NF_CLEAR_PATH)
                        || navigator.GetBestNodeAltRoute(NPC->waypoint, found->waypoint) ==
                           WAYPOINT_NONE) {//can't possibly have a route to any OR can't possibly have a route to this one OR don't have a route to this one
                        if (NAV_ClearPathToPoint(NPC, NPC->mins, NPC->maxs, found->currentOrigin,
                                                 NPC->clipmask,
                                                 ENTITYNUM_NONE)) {//have a clear straight path to this one
                            bestDist = dist;
                            bestFound = found;
                        }
                    } else {//can nav to it
...
void NPC_CheckGetNewWeapon(void) {
    if (NPC->s.weapon == WP_NONE && NPC->enemy) {//if running away because dropped weapon...
        if (NPCInfo->goalEntity
            && NPCInfo->goalEntity == NPCInfo->tempGoal
            && NPCInfo->goalEntity->enemy
            &&
            !NPCInfo->goalEntity->enemy->inuse) {//maybe was running at a weapon that was picked up
            NPCInfo->goalEntity = NULL;
        }
        if (TIMER_Done(NPC, "panic") &&
            NPCInfo->goalEntity == NULL) {//need a weapon, any lying around?
            gentity_t *foundWeap = NPC_SearchForWeapons();
            if (foundWeap) {//try to nav to it
                /*
...
void NPC_AimAdjust(int change) {
    ...
    if (TIMER_Done(NPC, "aimDebounce")) {
        ...
        if (NPCInfo->currentAim > NPCInfo->stats.aim + 5) {//can never be better than max aim
            NPCInfo->currentAim = NPCInfo->stats.aim + 5;
        } else if (NPCInfo->currentAim < -15) {//can never be worse than this
...
void G_AimSet(gentity_t *self, int aim) {
    if (self->NPC) {
        self->NPC->currentAim = (aim > 0) ? (aim * 2) : aim;
