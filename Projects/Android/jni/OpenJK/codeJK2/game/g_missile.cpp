...
static void G_MissileStick(gentity_t *missile, gentity_t *other, trace_t *tr) {
    if (other->NPC || !Q_stricmp(other->classname, "misc_model_breakable")) {
        ...
        if (tr->entityNum >= 0 && tr->entityNum < ENTITYNUM_WORLD &&
            tr->plane.normal[2] > 0.7 && missile->s.pos.trDelta[2] <
                                         40) //this can happen even on very slightly sloped walls, so changed it from > 0 to > 0.7
        {
    ...
    if (other->s.eType == ET_MOVER || other->e_DieFunc == dieF_funcBBrushDie ||
        other->e_DieFunc == dieF_funcGlassDie) {
...
void G_ReflectMissile(gentity_t *ent, gentity_t *missile, vec3_t forward) {
    ...
    if (ent && owner && owner->client && !owner->client->ps.saberInFlight &&
        ...
        if (enemy) {
            ...
            if (!PM_SaberInParry(owner->client->ps.saberMove)
                ...
                && !PM_SaberInIdle(owner->client->ps.saberMove)) {//a bit more wild
                if (PM_SaberInAttack(owner->client->ps.saberMove)
                    || PM_SaberInTransitionAny(owner->client->ps.saberMove)
                    || PM_SaberInSpecialAttack(owner->client->ps.torsoAnim)) {//moderately more wild
                    for (i = 0; i < 3; i++) {
                        bounce_dir[i] += Q_flrand(-0.2f, 0.2f);
                    }
                } else {//mildly more wild
                    for (i = 0; i < 3; i++) {
    ...
    if (!reflected) {
        if (missile->owner && missile->s.weapon != WP_SABER) {//bounce back at them if you can
        ...
        if (owner->s.weapon == WP_SABER && owner->client) {//saber
            if (owner->client->ps.saberInFlight) {//reflecting off a thrown saber is totally wild
                for (i = 0; i < 3; i++) {
                    bounce_dir[i] += Q_flrand(-0.8f, 0.8f);
                }
            } else if (owner->client->ps.forcePowerLevel[FP_SABER_DEFENSE] <=
                       FORCE_LEVEL_1) {// at level 1
                for (i = 0; i < 3; i++) {
            ...
            if (!PM_SaberInParry(owner->client->ps.saberMove)
                ...
                if (PM_SaberInAttack(owner->client->ps.saberMove)
                    || PM_SaberInTransitionAny(owner->client->ps.saberMove)
                    || PM_SaberInSpecialAttack(owner->client->ps.torsoAnim)) {//really wild
                    for (i = 0; i < 3; i++) {
        ...
        } else {//some other kind of reflection
            for (i = 0; i < 3; i++) {
                bounce_dir[i] += Q_flrand(-0.2f, 0.2f);
            }
        }
    ...
    if (missile->s.weapon != WP_SABER) {//you are mine, now!
        if (!missile->lastEnemy) {//remember who originally shot this missile
...
void G_BounceMissile(gentity_t *ent, trace_t *trace) {
    ...
    if (ent->s.eFlags & EF_BOUNCE_SHRAPNEL) {
        ...
        // check for stop
        if (trace->plane.normal[2] > 0.7 && ent->s.pos.trDelta[2] <
                                            40) //this can happen even on very slightly sloped walls, so changed it from > 0 to > 0.7
        {
            G_SetOrigin(ent, trace->endpos);
        ...
        if (trace->plane.normal[2] > 0.7 && ent->s.pos.trDelta[2] <
                                            40) //this can happen even on very slightly sloped walls, so changed it from > 0 to > 0.7
        {
            if (ent->s.weapon == WP_THERMAL) {//roll when you "stop"
    ...
    if (hitTime >= level.time) {//trace fraction must have been 1
        ent->s.pos.trTime = level.time - 10;
    } else {
        ent->s.pos.trTime = hitTime -
                            10; // this is kinda dumb hacking, but it pushes the missile away from the impact plane a bit
    }
#endif
    ...
    if (ent->s.weapon != WP_SABER
        ...
        && ent->e_ThinkFunc != thinkF_LimbThink) {//not a saber, bouncing thermal or limb
...
void G_MissileImpacted(gentity_t *ent, gentity_t *other, vec3_t impactPos, vec3_t normal,
                       int hitLoc = HL_NONE) {
    // impact damage
    if (other->takedamage) {
        ...
        if (ent->damage) {
            ...
            if (other->client) {
                ...
                if (npc_class == CLASS_SEEKER || npc_class == CLASS_PROBE ||
                    ...
                    npc_class == CLASS_MARK1 || npc_class == CLASS_MARK2 ||
...
void G_MissileImpact(gentity_t *ent, trace_t *trace, int hitLoc = HL_NONE) {
    ...
    if (ent->owner && (other->takedamage || other->client)) {
        if (!ent->lastEnemy || ent->lastEnemy ==
                               ent->owner) {//a missile that was not reflected or, if so, still is owned by original owner
    ...
    if (bounce) {
        ...
        if (other->NPC) {
            G_Damage(other, ent, ent->owner, ent->currentOrigin, ent->s.pos.trDelta, 0,
                     DAMAGE_NO_DAMAGE, MOD_UNKNOWN);
        }
    ...
    if ((!other->takedamage && (ent->s.eFlags & (EF_BOUNCE_SHRAPNEL)))
        ...
        if (!(other->contents & CONTENTS_LIGHTSABER)
            ...
            || (g_spskill->integer == 1 && ent->s.weapon != WP_FLECHETTE &&
                ent->s.weapon != WP_DEMP2)//on medium it won't reflect flechette or demp shots
            || (g_spskill->integer >= 2 && ent->s.weapon != WP_FLECHETTE &&
                ent->s.weapon != WP_DEMP2 && ent->s.weapon != WP_BOWCASTER && ent->s.weapon !=
                                                                              WP_REPEATER)//on hard it won't reflect flechette, demp, repeater or bowcaster shots
                ) {
            G_BounceMissile(ent, trace);
    ...
    if ((!other->takedamage || (other->client && other->health <= 0))
        && ent->s.weapon == WP_THERMAL
        && !ent->alt_fire) {//rolling thermal det - FIXME: make this an eFlag like bounce & stick!!!
    ...
    if (ent->s.eFlags & EF_MISSILE_STICK) {
        if (ent->owner)//&& ent->owner->s.number == 0 )
        {
            ...
            if (ent->s.weapon == WP_TRIP_MINE) {
                AddSoundEvent(ent->owner, ent->currentOrigin, ent->splashRadius / 2,
                              AEL_DISCOVERED/*AEL_DANGER*/ );
                AddSightEvent(ent->owner, ent->currentOrigin, ent->splashRadius * 2,
                              AEL_DISCOVERED/*AEL_DANGER*/, 60);
                /*
                AddSoundEvent( ent->owner, ent->currentOrigin, ent->splashRadius*2, AEL_DANGER );
    ...
    if (other->contents & CONTENTS_LIGHTSABER) {
        ...
        if ((g_spskill->integer <= 0//on easy, it reflects all shots
            ...
            && (!ent->splashDamage ||
                !ent->splashRadius))//this would be cool, though, to "bat" the thermal det away...
        {
            ...
            if (!other->owner || !other->owner->client || other->owner->client->ps.saberInFlight ||
                InFront(ent->currentOrigin, other->owner->currentOrigin,
                        other->owner->client->ps.viewangles,
                        SABER_REFLECT_MISSILE_CONE))//other->owner->s.number != 0 ||
            {//Jedi cannot block shots from behind!
                if ((other->owner->client->ps.forcePowerLevel[FP_SABER_DEFENSE] > FORCE_LEVEL_1 &&
                     Q_irand(0, 3))
                    ||
                    (other->owner->client->ps.forcePowerLevel[FP_SABER_DEFENSE] > FORCE_LEVEL_0 &&
                     Q_irand(0,
                             1))) {//level 1 reflects 50% of the time, level 2 reflects 75% of the time
        ...
        } else {//still do the bounce effect
            G_MissileReflectEffect(ent, trace->endpos, trace->plane.normal);
        }
    }
...
void G_RunStuckMissile(gentity_t *ent) {
    if (ent->takedamage) {
        if (ent->s.groundEntityNum >= 0 && ent->s.groundEntityNum < ENTITYNUM_WORLD) {
            ...
            if ((!VectorCompare(vec3_origin, other->s.pos.trDelta) &&
                 other->s.pos.trType != TR_STATIONARY) ||
                (!VectorCompare(vec3_origin, other->s.apos.trDelta) && other->s.apos.trType !=
                                                                       TR_STATIONARY)) {//thing I stuck to is moving or rotating now, kill me
...
void G_RollMissile(gentity_t *ent) {
    ...
    if (objPML.groundPlane) {//FIXME: never happens!
        // slide along the ground plane
        PM_ClipVelocity(ent->s.pos.trDelta, objPML.groundTrace.plane.normal, ent->s.pos.trDelta,
                        BUMPCLIP);
    ...
    */
    ...
    for (bumpcount = 0; bumpcount < numbumps; bumpcount++) {
        ...
        gi.trace(&trace, ent->currentOrigin, ent->mins, ent->maxs, end, ent->s.number,
                 ent->clipmask, G2_RETURNONHIT, 10);
        ...
        if (trace.entityNum < ENTITYNUM_WORLD) {//hit another ent
            ...
            if (hitEnt && (hitEnt->takedamage || (hitEnt->contents & CONTENTS_LIGHTSABER))) {
                ...
                if (ent->s.eType == ET_GENERAL) {//exploded
                    return;
                }
            }
        }
        ...
        */
        ...
        //
        if (&g_entities[trace.entityNum] != NULL &&
            g_entities[trace.entityNum].client) {//hit a person, bounce off much less
            bounceAmt = OVERCLIP;
        } else {
            bounceAmt = BUMPCLIP;
        }
...
void G_RunMissile(gentity_t *ent) {
    ...
    if (ent->s.pos.trType == TR_INTERPOLATE) {//rolling missile?
        //FIXME: WTF?!!  Sticks to stick missiles?
        //FIXME: they stick inside the player
        G_RollMissile(ent);
        if (ent->s.eType != ET_GENERAL) {//didn't explode
            VectorCopy(ent->currentOrigin, ent->s.pos.trBase);
            gi.trace(&tr, oldOrg, ent->mins, ent->maxs, ent->currentOrigin, ent->s.number,
                     ent->clipmask, G2_RETURNONHIT, 10);
        ...
        */
        ...
        if (tr.entityNum != ENTITYNUM_NONE && &g_entities[tr.entityNum] != NULL) {
            ...
            // check for hitting a lightsaber
            if (other->contents & CONTENTS_LIGHTSABER) {//hit a lightsaber bbox
                if (other->owner && other->owner->client &&
                ...
                {//Jedi cannot block shots from behind!
                    ...
                    gi.trace(&tr, tr.endpos, ent->mins, ent->maxs, origin, tr.entityNum,
                             ent->clipmask, G2_RETURNONHIT, 10);
                }
            }
        }
    ...
    // get current angles
    VectorMA(ent->s.apos.trBase, (level.time - ent->s.apos.trTime) * 0.001, ent->s.apos.trDelta,
             ent->s.apos.trBase);
...
//				hitModel = qtrue;
                ...
                if (trHitLoc == HL_NONE) {
                    G_GetHitLocFromSurfName(&g_entities[coll.mEntityNum], gi.G2API_GetSurfaceName(
                                                    &g_entities[coll.mEntityNum].ghoul2[coll.mModelIndex],
                                                    coll.mSurfaceIndex), &trHitLoc, coll.mCollisionPosition, NULL, NULL,
                                            ent->methodOfDeath);
                }
...
/////////////////////////////////////////////////////////
    ...
    if (ent->s.pos.trType == TR_STATIONARY &&
        (ent->s.eFlags & EF_MISSILE_STICK)) {//stuck missiles should check some special stuff
    ...
    */
    ...
    AddSightEvent(ent->owner, ent->currentOrigin, 512, AEL_DISCOVERED,
                  75);//wakes them up when see a shot passes in front of them
    if (!Q_irand(0, 10)) {//not so often...
        if (ent->splashDamage &&
            ent->splashRadius) {//I'm an exploder, let people around me know danger is coming
            if (ent->s.weapon == WP_TRIP_MINE) {//???
            } else {
                if (ent->s.weapon == WP_ROCKET_LAUNCHER &&
                    ent->e_ThinkFunc == thinkF_rocketThink) {//homing rocket- run like hell!
                    AddSightEvent(ent->owner, ent->currentOrigin, ent->splashRadius,
                                  AEL_DANGER_GREAT, 50);
                } else {
                    AddSightEvent(ent->owner, ent->currentOrigin, ent->splashRadius, AEL_DANGER,
                                  50);
                }
                AddSoundEvent(ent->owner, ent->currentOrigin, ent->splashRadius, AEL_DANGER);
            }
        } else {//makes them run from near misses
